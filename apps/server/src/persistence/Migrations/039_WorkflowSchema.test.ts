 ...
 * plus pin_order_key from upstream's 038_ProjectionThreadsPinOrderKey,
 * which now precedes this migration (2026-07-22 rebase renumber 033→034). The consolidated migration
 * 039_WorkflowSchema (formerly 033/034/035/037; renumbered again when upstream took slots 37-38) must reproduce it EXACTLY. The dump filters to
 ...
 * folded into the consolidated workflow migration under the single-migration rule
 * (the standalone migration that once carried it is deleted),
 * added by 036_DispatchKind for agent-question continuations and registered as
 * id 39 in this rebased loader. It is listed in GOLDEN because this dump runs
...
layer("039_WorkflowSchema", (it) => {
  it.effect("migration entry exists at id 39", () =>
    Effect.gen(function* () {
      assert.isTrue(migrationEntries.some(([id, name]) => id === 39 && name === "WorkflowSchema"));
    }),
  );
  ...
  it.effect("collapsed schema equals the golden 033->055 chain schema", () =>
    Effect.gen(function* () {
      ...
      // toMigrationInclusive cannot roll back later ids. Exclude post-039 tables
      // that match the workflow_% filter.
      ...
      // Strip later ALTER columns/indexes that land on projection_ticket when
      // full MigrationsLive has already advanced past the collapsed workflow schema.
  ...
  it.effect("partial open index on workflow_pr_state present", () =>
    Effect.gen(function* () {
      ...
      const indexes = yield* sql<{
        readonly name: string;
      }>`PRAGMA index_list(workflow_pr_state)`;
  ...
  /**
   * Guards the collapse: 039 must remain the branch's single consolidated
   * workflow migration, and anything after it must be genuinely NEW schema
   * rather than an edit smuggled into 039.
   *
   * 039 itself can never be edited again — a database that already applied it
   * would never receive the change, and every dispatch INSERT would fail on the
   * missing column. So new work adds a migration on top of 039.
   */
  it.effect("039 stays the consolidated workflow migration", () =>
    Effect.gen(function* () {
