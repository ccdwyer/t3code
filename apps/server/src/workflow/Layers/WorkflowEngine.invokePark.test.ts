  ...
  it.effect("returns stale for a ticket that is not parked and emits nothing", () =>
    Effect.gen(function* () {
      ...
      yield* registry.register(
        ...
        {
          name: "idle-wf",
          lanes: [{ key: "hold", name: "Hold", entry: "manual" }],
        } as never,
      );
      const engine = yield* WorkflowEngine;
...
const mismatchExecutor = makeScriptedExecutor(() => ({
  _tag: "failed",
  error: "boom",
}));
...
const raceExecutor = makeScriptedExecutor(() => ({
  _tag: "failed",
  error: "boom",
}));
...
const deletedTargetExecutor = makeScriptedExecutor(() => ({
  _tag: "failed",
  error: "boom",
}));
...
const oobExecutor = makeScriptedExecutor(() => ({
  _tag: "failed",
  error: "boom",
}));
...
const nullOriginExecutor = makeScriptedExecutor(() => ({
  _tag: "failed",
  error: "boom",
}));
