...
const expectDecodable = async (definition: WorkflowDefinitionEncodedType) => {
  const encoded = await Effect.runPromise(decodeEditorDefinition(definition));
  await Effect.runPromise(decodeWorkflowDefinition(encoded));
};

describe("workflow editor model", () => {
  it("tracks dirty state, lint errors, saved baselines, and discards", async () => {
    ...
    await expectDecodable(discarded.definition);
  });
  ...
  it("mutates lanes immutably with unique keys and decodable encoded output", async () => {
    ...
    expect(model.definition.lanes.some((candidate) => candidate.key === "new-lane-2")).toBe(false);
    await expectDecodable(model.definition);
  });

  it("adds, updates, reorders, and removes steps with decodable defaults", async () => {
    let model = createWorkflowEditorModel(baseDefinition);
  ...
  it("mutates lane routing and transitions with parsed transition predicates", async () => {
    let model = createWorkflowEditorModel(baseDefinition);
  ...
  it("mutates lane external-event matchers", async () => {
    let model = createWorkflowEditorModel(baseDefinition);
  ...
  it("drops lane action and event targets referencing a removed lane", async () => {
    let model = createWorkflowEditorModel(baseDefinition);
