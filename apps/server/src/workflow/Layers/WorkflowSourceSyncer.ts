...
const make = Effect.gen(function* () {
  ...
  const sweep: WorkflowSourceSyncerShape["sweep"] = Effect.gen(function* () {
    const definitions = yield* boards.listDefinitions().pipe(
      Effect.catchCause((cause) =>
        Effect.logWarning("workflow.source-syncer.list-boards-failed", {
          cause,
        }).pipe(Effect.as([] as ReadonlyArray<{ readonly boardId: BoardId }>)),
      ),
    );
    ...
    for (const { boardId, definition } of definitions as ReadonlyArray<{
      ...
      readonly definition: {
        readonly sources?: ReadonlyArray<WorkflowSourceConfig>;
      };
    }>) {
      ...
      yield* engine.recoverBoardWip(boardId).pipe(
        Effect.catchCause((cause) =>
          Effect.logWarning("workflow.source-syncer.recover-wip-failed", {
            boardId,
            cause,
          }),
        ),
      ...
      yield* Effect.forkScoped(
        sweep.pipe(
          Effect.catchDefect((defect: unknown) =>
            Effect.logWarning("workflow.source-syncer.sweep-defect", {
              defect,
            }),
          ),
