...
function boardTicketState(
  overrides: Partial<RelayBoardTicketState> = {},
...
type EnqueueArgs = Parameters<ApnsDeliveryQueue.ApnsDeliveryQueueShape["enqueuePushNotification"]>[0];
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        Layer.succeed(
          ...
          makeApnsDeliveryQueue(input.capture),
        ),
...
describe("BoardTicketPublisher", () => {
    ...
    () => {
      ...
      const state = boardTicketState({
        ...
        transitionId: ("transition-" +
          "e".repeat(36)) as RelayBoardTicketState["transitionId"],
      });
      ...
      return Effect.gen(function* () {
        ...
        Effect.provide(
          provide({
            targets: [
              target({ device_id: longDeviceId, preferences_json: preferences() }),
            ],
            capture,
          }),
        ),
  ...
  it.effect("does not enqueue when notifyOnApproval is false for a waiting_for_approval state", () => {
    const capture: Array<EnqueueArgs> = [];
  ...
  it.effect("returns no deliveries and enqueues nothing for a cleared (null) state", () => {
    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(provide({ targets: [target()], capture })),
    );
  });
  ...
  it.effect("produces distinct bounded jobIds for tuples differing only by colon placement", () => {
    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        provide({
          targets: [
            target({ device_id: "device:with:colons", preferences_json: preferences() }),
          ],
          capture,
        }),
      ),
