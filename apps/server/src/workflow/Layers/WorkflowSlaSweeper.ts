...
const makeWorkflowSlaSweeper = (options?: WorkflowSlaSweeperLiveOptions) =>
  Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        const boards = yield* boardRegistry.listDefinitions().pipe(
          Effect.catchCause((cause) =>
            Effect.logWarning("workflow.sla-sweeper.list-definitions-failed", {
              cause,
            }).pipe(
              Effect.as(
                [] as ReadonlyArray<{
     ...
     */
    const resumeStrandedQuestions = engine.resumeAnsweredQuestions().pipe(
      Effect.catchCause((cause) =>
        Effect.logWarning("workflow.sla-sweeper.resume-questions-failed", {
          cause,
        }),
      ),
    );
