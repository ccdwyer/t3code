...
const makeLayer = (params: {
  ...
  WorkflowSourceSyncerLive.pipe(
    ...
    Layer.provide(
      Layer.succeed(
        WorkflowEngine,
        recordingEngine(params.recoveries, {
          recoverFails: params.recoverFails ?? false,
        }),
      ),
    ),
      ...
      const scripts = new Map<string, ProviderScript>([
        [
          "src-err",
          {
            ...
            getItemFailWith: new WorkSourceTransientError({
              message: "github 500 (getItem)",
            }),
          },
        ],
      ]);
  ...
  () =>
    Effect.gen(function* () {
      ...
      const boards = [
        board(
          "board-1",
          githubSource("source-throttle", "src-throttle", true, {
            syncIntervalSec: 600,
          }),
        ),
      ];
...
it.effect("Fix 4: a source with a STALE last_full_run_at (older than the interval) RUNS", () =>
  Effect.gen(function* () {
    ...
    const run = Effect.gen(function* () {
      ...
      yield* seedState("board-1", "source-due", {
        lastFullRunAt: "2000-01-01T00:00:00Z",
      });
      const syncer = yield* WorkflowSourceSyncer;
      yield* syncer.sweep;
