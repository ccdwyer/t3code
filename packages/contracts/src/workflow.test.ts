...
describe("AgentStep continueSession", () => {
  ...
  it.effect("decodes an agent step with and without continueSession", () =>
    Effect.gen(function* () {
      ...
      const withTrue = yield* decodeAgentStep({
        ...base,
        continueSession: true,
      });
      assert.equal(withTrue.continueSession, true);

      const withFalse = yield* decodeAgentStep({
        ...base,
...
describe("WorkflowEvent", () => {
  ...
  it.effect("carries structured output on completed steps and step run views", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual((outcome as any).output, {
        verdict: "pass",
        score: 0.98,
      });
      ...
      assert.deepEqual((event.payload as any).output, {
        verdict: "pass",
        score: 0.98,
      });
      ...
      assert.deepEqual((view as any).output, {
        verdict: "pass",
        score: 0.98,
      });
    }),
...
describe("MergeStep", () => {
  ...
  it.effect("decodes non-retryable failed step outcomes", () =>
    Effect.gen(function* () {
      ...
      const legacy = yield* decodeStepOutcome({
        _tag: "failed",
        error: "boom",
      });
        ...
        lintErrors: [
          {
            ...
            message: "Duplicate step key 'code'",
          },
        ],
      });
      assert.equal((result as any).ok, false);
