...
const decodeBody = (
  ...
  if (body._tag === "Uint8Array") {
    const u8 = body as unknown as {
      readonly body: Uint8Array;
      readonly contentType: string;
    };
    return {
      bodyText: new TextDecoder().decode(u8.body),
      contentType: u8.contentType,
    };
  }
  ...
  Layer.succeed(WorkflowOutboundConnectionStore, {
    getTarget: (connectionRef: string) => {
      ...
      if (entry === undefined || entry === "missing") {
        return Effect.fail(
          new OutboundConfigError({
            reason: `no connection for ${connectionRef}`,
          }),
        );
      }
...
const WEBHOOK_TARGET: OutboundTarget = {
  kind: "webhook",
  url: "https://hooks.example.com/x",
};
const SLACK_TARGET: OutboundTarget = {
  kind: "slack",
...
describe.sequential("WorkflowOutboundDispatcher", () => {
  ...
  it.effect("two concurrent drains of the same pending row POST it exactly once", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* Effect.all([dispatcher.sweep(), dispatcher.sweep()], {
        concurrency: 2,
      });
