...
import { TicketAttachment } from "@t3tools/contracts";
...
const warnUnrecognizedPrStates = (rows: ReadonlyArray<TicketDependencySqlRow>) => {
  const ticketIds = rows
    .filter(
      (row) =>
        typeof row.prState === "string" && !PR_STATES.some((state) => state === row.prState),
    )
...
function withDependencyFields(row: TicketDependencySqlRow | null): TicketRow | null {
  ...
  const { dependsOnJson, prNumber, prUrl, prState, prCiState, sourceMetadataJson: _sm, ...ticket } = row;
...
function parseSyncedSource(
  ...
  try {
    ...
    if (Array.isArray(obj["assignees"])) {
      result.assignees = obj["assignees"].filter(
...
const make = Effect.gen(function* () {
  ...
  const getTicketDetail: WorkflowReadModelShape["getTicketDetail"] = (ticketId) =>
    Effect.gen(function* () {
      ...
      const currentLane = yield* resolveCurrentLane(
        rawTicket.boardId,
        rawTicket.currentLaneKey,
      );
