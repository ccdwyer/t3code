...
describe("contentStateFor", () => {
  ...
  it("unavailable wins over everything else", () => {
    expect(
      contentStateFor(view({ content: "stale", contentTruncated: true, contentUnavailable: true })),
...
describe("artifactsInServerOrder", () => {
  it("preserves the server's canonical order — never re-sorts", () => {
    ...
    const result = { artifacts, scratch: [] } as unknown as WorkflowTicketArtifactsResult;
