...
describe("SidebarV2 mode switch", () => {
  ...
  it("clears thread selection only on Threads → Workflows", () => {
    expect(shouldClearThreadSelectionOnModeChange({ from: "threads", to: "workflows" })).toBe(true);
    expect(shouldClearThreadSelectionOnModeChange({ from: "workflows", to: "threads" })).toBe(
      false,
    );
    expect(shouldClearThreadSelectionOnModeChange({ from: "threads", to: "threads" })).toBe(false);
  });
});
