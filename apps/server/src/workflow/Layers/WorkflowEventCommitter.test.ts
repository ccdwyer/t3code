  ...
  () =>
    Effect.gen(function* () {
      ...
      const fakeStore = Layer.succeed(WorkflowEventStore, {
        ...
        readByBoard: unsupportedStream,
        readTicketTail: unsupportedStream,
        readTicketRange: unsupportedStream,
        maxSequenceForBoard: () => Effect.die("unused"),
...
layer("WorkflowEventCommitter", (it) => {
  ...
  it.effect("writes exactly one outbox row when an event flips a ticket into waiting_on_user", () =>
    Effect.gen(function* () {
      ...
      const eventRows = yield* (yield* SqlClient.SqlClient)<{
        readonly sequence: number;
      }>`
