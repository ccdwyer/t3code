...
const stageBounded = async (
  ...
  | {
      readonly ok: false;
      readonly reason: "over-file-cap" | "unstable-source" | "unreadable";
    }
> => {
    ...
    for (const loser of names) {
      if (loser !== winner) {
        skips.push({
          ...
          detail: "duplicate sidecar casing",
        });
      }
    }
    if (!fileSet.has(base)) {
  ...
  for (const { raw, normalized } of preliminary) {
    ...
    if (group.length > 1 && collisionWinner(group.map((g) => g.raw)) !== raw) {
      skips.push({
        name: raw,
        reason: "name-collision",
        detail: "normalizes to a duplicate name",
      });
      continue;
    }
        ...
        Effect.promise(async () => {
          for (const item of staged) {
            if (item.mode === "staged" && item.blobId !== undefined && !keep.has(item.blobId)) {
              await Fs.rm(NodePath.join(dir, item.blobId), {
                force: true,
              }).catch(() => undefined);
            }
          }
            ...
            for (const item of staged) {
              ...
              if (item.mode === "unchanged") {
                if (row === null) {
                  ...
                  allSkips.push({
                    name: item.candidate.rawName,
                    reason: "unstable-source",
                  });
                  continue;
                }
        ...
        for (const item of staged) {
          if (item.mode === "staged" && item.blobId !== undefined) {
            yield* Effect.promise(() =>
              Fs.rm(NodePath.join(dir, item.blobId ?? ""), {
                force: true,
              }).catch(() => undefined),
            );
          }
