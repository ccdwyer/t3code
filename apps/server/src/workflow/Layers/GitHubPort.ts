  ...
  const resolveRemote: GitHubPortShape["resolveRemote"] = (cwd) =>
    registry.resolveHandle({ cwd }).pipe(
      ...
      Effect.flatMap((handle) => {
        ...
        return gh
          .getRepositoryCloneUrls({ cwd, repository: context.remoteName })
          .pipe(
  ...
  const openPr: GitHubPortShape["openPr"] = (input) =>
    Effect.gen(function* () {
      ...
      const push = yield* git
        ...
        .pipe(
          Effect.mapError((error) => eventStoreError("failed to push branch", error)),
        );
      if (push.exitCode !== 0) {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          const bodyFile = yield* fileSystem
            .makeTempFileScoped({ prefix: "t3-pr-body-" })
            .pipe(
  ...
  const failingCheckLogs: GitHubPortShape["failingCheckLogs"] = (input) =>
    Effect.gen(function* () {
      ...
      if (runId === undefined) {
        ...
        return failing.map((check) => check.name).filter((name) => name.length > 0).join(", ");
      }
