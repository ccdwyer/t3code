  ...
  lanes: [
    {
      ...
      name: "Review",
      entry: "auto",
      pipeline: [{ key: "gate", type: "approval", prompt: "Review it", ...(form ? { form } : {}) }],
...
layer("WorkflowEngine checkpoint forms", (it) => {
  it.effect("routes by the outcome the chosen decision maps to, for all three outcomes", () =>
    Effect.gen(function* () {
      ...
      for (const [board, decision, lane] of [
        ...
        yield* engine.resolveApproval(stepRunId as never, { approved: false, decision });
  ...
  it.effect("snapshots the form on the wait so a later board edit cannot change it", () =>
    Effect.gen(function* () {
      ...
      const engine = yield* WorkflowEngine;
      yield* engine.resolveApproval(stepRunId as never, { approved: false, decision: "ship" });
      const detail = yield* awaitTicketWhere(
        ticketId as string,
  ...
  it.effect("rejects a decision that is not on the snapshot", () =>
    Effect.gen(function* () {
      ...
      const failure = yield* engine
        .resolveApproval(stepRunId as never, { approved: true, decision: "merge-anyway" })
        .pipe(Effect.flip);
