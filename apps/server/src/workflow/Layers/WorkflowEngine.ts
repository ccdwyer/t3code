...
const make = Effect.gen(function* () {
      ...
      if (step.type === "approval") {
        ...
        if (resolution.outcome === "blocked") {
          ...
          yield* commit({
            ...
            payload: {
              stepRunId,
              reason: resolution.decision ?? "checkpoint blocked",
            },
          });
          return {
            result: "blocked",
      ...
      if (step.type === "fork") {
        if (Option.isNone(forkJoinOption)) {
          ...
          return {
            result: "failed",
            noRetry: true,
            detail: "fork join coordinator unavailable",
          };
        }
  ...
  const settleForkChildIfAny = (
    ...
    Effect.gen(function* () {
      ...
      const pipelineRows = yield* wrapSql(sql<{
      ...
      return yield* withAdmissionLock(
        ...
        Effect.gen(function* () {
          ...
          if (canonical.length === 0) {
            ...
            return {
              sections: [] as ReadonlyArray<WorkflowContextPackSection>,
            };
          }
  ...
  const steerTicketStep: WorkflowEngineShape["steerTicketStep"] = (input) =>
    Effect.gen(function* () {
      ...
      if (existingSteers[0] !== undefined) {
        ...
        return yield* new WorkflowEventStoreError({
          message: STEER_REJECTION.messageIdReuse,
        });
      }
      ...
      if (detail === null) {
        return yield* new WorkflowEventStoreError({
          message: STEER_REJECTION.ticketNotFound,
        });
      }
      if (detail.ticket.status === "parked") {
        return yield* new WorkflowEventStoreError({
          message: STEER_REJECTION.parkedTicket,
        });
      }
      ...
      if (step === undefined) {
        return yield* new WorkflowEventStoreError({
          message: STEER_REJECTION.stepNotFound,
        });
      }
      if (step.stepType !== "agent") {
        return yield* new WorkflowEventStoreError({
          message: STEER_REJECTION.notAgentStep,
        });
      }
      if (step.status === "dispatch_requested") {
        return yield* new WorkflowEventStoreError({
          message: STEER_REJECTION.agentStarting,
        });
      }
      if (step.status === "awaiting_user") {
        return yield* new WorkflowEventStoreError({
          message: STEER_REJECTION.awaitingUser,
        });
      }
      if (step.status !== "running") {
        return yield* new WorkflowEventStoreError({
          message: STEER_REJECTION.stepNotRunning,
        });
      }
      ...
      if (target.panelSize !== null && target.panelSize >= 2) {
        return yield* new WorkflowEventStoreError({
          message: STEER_REJECTION.panelStep,
        });
      }
      if (target.steerPendingMessageId !== null) {
        return yield* new WorkflowEventStoreError({
          message: STEER_REJECTION.steerInFlight,
        });
      }
  ...
  const answerTicketStep: WorkflowEngineShape["answerTicketStep"] = (input) =>
    Effect.gen(function* () {
      ...
      const resumedLiveWaiter = yield* approvals.resolve(input.stepRunId, {
        outcome: "success",
      });
      ...
      const lockOutcome = yield* withAdmissionLock(
        ...
        Effect.uninterruptible(
          Effect.gen(function* () {
            ...
            const termRows = yield* wrapSql(sql<{
            ...
            const revalidate = Effect.gen(function* () {
              ...
              const enteredRows = yield* wrapSql(sql<{
  ...
  const recoverQuestionRaise = (
    ...
    Effect.gen(function* () {
      ...
      if (!dispatch.ok) {
        return {
          kind: "unmappable",
          message: "could not read the step's dispatch",
        };
      }
      ...
      const turn = {
        threadId: dispatchRow.threadId,
        turnId: dispatchRow.turnId,
      };
      ...
      if (!strict.ok) {
        return {
          kind: "unmappable",
          message: "could not read the turn's final message",
        };
      }
    ...
    Effect.gen(function* () {
      ...
      const supersedePipeline = commitMany([
        ...
        // user re-routes it manually once the board matches reality again.
        {
          ...
          ticketId: recovered.stepStarted.ticketId,
          payload: {
            reason: "board definition changed while this step was recovering",
          },
        },
  ...
  const resumeRecoveredQuestion = (
    ...
    Effect.gen(function* () {
      ...
      if (!claimed) {
        ...
        // The holder may have finished it while we waited.
        const settled = yield* readStoredEventsForStep(stepRunId).pipe(
          Effect.orElseSucceed(() => null),
        );
        if (settled !== null && hasTerminalStepEvent(settled, stepRunId)) {
          yield* releaseRecoveredStepClaim(stepRunId);
          return;
        }
      }
    ...
    Effect.gen(function* () {
      ...
      for (const candidate of candidates) {
        ...
        yield* resumeRecoveredQuestion(
          {
            ticketId: latestWait.ticketId,
            payload: latestWait.payload,
          } as PendingWait,
          answered,
          recovered,
          true,
