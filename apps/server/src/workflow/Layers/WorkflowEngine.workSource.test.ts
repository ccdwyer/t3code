...
layer("WorkflowEngine work_source unlocked ops", (it) => {
  ...
  it.effect("editTicketFieldsUnlocked does not blank the stored title for a whitespace-only title", () =>
    Effect.gen(function* () {
      const registry = yield* BoardRegistry;
  ...
  it.effect("Fix 1: editTicketFieldsUnlocked WRITES an empty-string description (clear), distinct from undefined (leave)", () =>
    Effect.gen(function* () {
      const registry = yield* BoardRegistry;
