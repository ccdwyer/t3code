...
layer("WorkflowReadModel", (it) => {
  ...
  it.effect(
    ...
    () =>
      ...
      `;
        ...
        yield* insertTicket({
          ticketId: "ticket-dep-b",
          queuedAt: "2026-06-07T00:00:01.000Z",
        });
        yield* insertTicket({
          ticketId: "ticket-dep-c",
    ...
    Effect.gen(function* () {
      ...
      const insertEvent = (input: {
        ...
        VALUES (
          ...
          ${input.eventType},
          ${input.occurredAt},
          ${encodeUnknownJsonString(input.payload)}
        )
      `;
      yield* insertEvent({
    ...
    Effect.gen(function* () {
      ...
      const insertRouteDecision = (payload: unknown) => sql`
        ...
        VALUES (
          ...
          ${encodeUnknownJsonString(payload)}
        )
      `;
      ...
      yield* Effect.forEach(
        ...
        (index) => sql`
          ...
          VALUES (
            ...
            ${encodeUnknownJsonString({ toLane: `lane-${index}`, laneEntryToken: `token-${index}`, reason: "manual" })}
          )
        `,
      );
  ...
  it.effect("returns completed step output in ticket detail", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual((detail?.steps[0] as any)?.output, {
        verdict: "pass",
        score: 0.98,
      });
    }),
  ...
  const seedPack = (ticketId: string, boardId: string, forLane: string) =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      ...
      yield* sql`
        ...
        VALUES (${ticketId}, ${boardId}, 'Packed', ${forLane}, 'running', ${now}, ${now})
      `;
      yield* sql`
        ...
        VALUES (
          ${ticketId}, ${forLane}, 'implement', ${now}, NULL,
          ${encodeUnknownJsonString([{ key: "notes", body: "hand off", autoGenerated: true }])}
        )
      `;
    });
  ...
  it.effect("getContextPack round-trips the stored sections", () =>
    Effect.gen(function* () {
      const read = yield* WorkflowReadModel;
      yield* seedPack("ticket-pack-read", "b-pack", "review");

      const pack = yield* read.getContextPack("ticket-pack-read" as never, "review" as never);
      assert.isNotNull(pack);
      assert.equal(pack?.fromLane, "implement");
      assert.equal(pack?.editedAt, undefined);
      assert.deepStrictEqual(pack?.sections, [
        { key: "notes", body: "hand off", autoGenerated: true },
      ]);

      // A pack is keyed per destination lane, so another lane sees nothing.
      const other = yield* read.getContextPack("ticket-pack-read" as never, "implement" as never);
      assert.isNull(other);
    }),
  );

  it.effect("getContextPack degrades to null when sections_json is not an array", () =>
    Effect.gen(function* () {
      const read = yield* WorkflowReadModel;
      const sql = yield* SqlClient.SqlClient;
      yield* seedPack("ticket-pack-corrupt", "b-pack", "review");
      // Corrupt rows must not blow up the ticketDetail wire encode.
      yield* sql`
        UPDATE projection_context_pack SET sections_json = '{"not":"an array"}'
        WHERE ticket_id = 'ticket-pack-corrupt'
      `;

      const pack = yield* read.getContextPack("ticket-pack-corrupt" as never, "review" as never);
      assert.isNull(pack);
    }),
  );

  it.effect("getContextPack degrades to null when a section key is not a contract literal", () =>
    Effect.gen(function* () {
      const read = yield* WorkflowReadModel;
      const sql = yield* SqlClient.SqlClient;
      yield* seedPack("ticket-pack-badkey", "b-pack", "review");
      // Shape-valid but not one of the four allowed keys. Passing it through
      // would move the failure to the wire encoder and fail the whole detail read.
      yield* sql`
        UPDATE projection_context_pack
        SET sections_json = '[{"key":"evil","body":"x","autoGenerated":true}]'
        WHERE ticket_id = 'ticket-pack-badkey'
      `;

      assert.isNull(yield* read.getContextPack("ticket-pack-badkey" as never, "review" as never));
    }),
  );

  it.effect("deleteTicketState removes the ticket's context packs", () =>
    Effect.gen(function* () {
      const read = yield* WorkflowReadModel;
      const sql = yield* SqlClient.SqlClient;
      yield* seedPack("ticket-pack-del", "b-pack-del", "review");

      yield* read.deleteTicketState("ticket-pack-del" as never);

      const remaining = yield* sql<{ readonly count: number }>`
        SELECT COUNT(*) AS count FROM projection_context_pack WHERE ticket_id = 'ticket-pack-del'
      `;
      assert.equal(remaining[0]?.count, 0);
    }),
  );

  it.effect("deleteBoardTicketState removes context packs for every ticket on the board", () =>
    Effect.gen(function* () {
      const read = yield* WorkflowReadModel;
      const sql = yield* SqlClient.SqlClient;
      yield* seedPack("ticket-pack-b1", "b-pack-cascade", "review");
      yield* seedPack("ticket-pack-b2", "b-pack-cascade", "verify");
      yield* seedPack("ticket-pack-keep", "b-pack-other", "review");

      yield* read.deleteBoardTicketState("b-pack-cascade" as never);

      // Scoped to this test's tickets: the layer's DB is shared across the file.
      const rows = yield* sql<{ readonly ticket_id: string }>`
        SELECT ticket_id FROM projection_context_pack
        WHERE ticket_id IN ('ticket-pack-b1', 'ticket-pack-b2', 'ticket-pack-keep')
        ORDER BY ticket_id
      `;
      assert.deepStrictEqual(
    ...
    Effect.gen(function* () {
      ...
      const insertEvent = (input: {
        ...
        VALUES (
          ...
          ${input.occurredAt}, ${encodeUnknownJsonString(input.payload)}
        )
      `;
      yield* insertEvent({
  ...
  it.effect("listBoardProposals returns proposals pending-first, computed outdated flag", () =>
    Effect.gen(function* () {
      ...
      const agentJson = encodeUnknownJsonString({
        instance: "claude",
        model: "sonnet",
      });
