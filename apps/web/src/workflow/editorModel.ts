...
export const PARK_SELECT_VALUES = {
  issue: "__park_issue",
  waiting: "__park_waiting",
} as const;
...
export type RouteTargetPath =
  | {
      readonly site: "laneOn";
      readonly laneKey: string;
      readonly kind: LaneRoutingKind;
    }
  | {
      readonly site: "transition";
      readonly laneKey: string;
      readonly index: number;
    }
  | {
      readonly site: "laneEvent";
      readonly laneKey: string;
      readonly index: number;
    }
  | {
...
export const addLane = (model: WorkflowEditorModel): WorkflowEditorModel =>
  mutateDefinition(model, (definition) => {
    ...
    definition.lanes.push({
      key: LaneKey.make(key),
      name: "New lane",
      entry: "manual",
    });
  });
