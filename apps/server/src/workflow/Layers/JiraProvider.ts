...
function requireBaseUrl(auth: ConnAuth): Effect.Effect<string, WorkSourceConfigError> {
  ...
  if (!base) {
    return Effect.fail(
      new WorkSourceConfigError({
        message: "Jira connection is missing a base URL",
      }),
    );
  }
  ...
  return Effect.try({
    try: () => new URL(base).hostname,
      ...
      new WorkSourceConfigError({
        message: "Jira base URL is not a valid URL",
      }),
  }).pipe(
...
function buildJql(
  selector: { projectKey: string; jql?: string | undefined },
  since?: string,
): string {
      ...
      Effect.gen(function* () {
        const selector = yield* decodeJiraSelector(input.selector).pipe(
          Effect.mapError(
            (e) =>
              new WorkSourceConfigError({
                message: `Invalid Jira selector: ${e.message}`,
              }),
          ),
        );
        ...
        const request = HttpClientRequest.get(`${baseUrl}/rest/api/2/search`, {
          urlParams,
        ...
        if (status === 429) {
          return yield* new WorkSourceRateLimitError({
            retryAfterMs: parseJiraRateLimitRetryMs(respHeaders),
          });
        }
        if (status === 401 || status === 403) {
          return yield* new WorkSourceAuthError({
        ...
        const raw = yield* response.json.pipe(
          Effect.mapError(
            (cause) =>
              new WorkSourceTransientError({
                message: `Failed to parse Jira JSON: ${String(cause)}`,
              }),
          ),
        );
      ...
      Effect.gen(function* () {
        ...
        const raw = yield* response.json.pipe(
          Effect.mapError(
            (cause) =>
              new WorkSourceTransientError({
                message: `Failed to parse Jira getItem JSON: ${String(cause)}`,
              }),
          ),
        );
        if (raw === null || typeof raw !== "object" || Array.isArray(raw)) {
