...
function makeWorkflowClient(environmentId: EnvironmentId): WorkflowClient {
  return {
    workflow: {
      ...
      getTicketDetail: (input) => runRpc(environmentId, WORKFLOW_WS_METHODS.getTicketDetail, input),
      answerTicketStep: (input) =>
        runRpc(environmentId, WORKFLOW_WS_METHODS.answerTicketStep, input),
      resolveApproval: (input) => runRpc(environmentId, WORKFLOW_WS_METHODS.resolveApproval, input),
