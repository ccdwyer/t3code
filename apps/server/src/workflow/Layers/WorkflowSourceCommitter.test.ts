...
layer("WorkflowSourceCommitter.reconcileChunk", (it) => {
  ...
  it.effect("change: differing content_hash edits the ticket + bumps the mapping; same hash is a no-op", () =>
    Effect.gen(function* () {
      const registry = yield* BoardRegistry;
  ...
  it.effect("close: a closed delta routes to closedLane with source work_source + mapping lifecycle closed", () =>
    Effect.gen(function* () {
      const registry = yield* BoardRegistry;
  ...
  it.effect("orphan: a missing delta marks sync_status orphaned; confirmedDeleted also closes", () =>
    Effect.gen(function* () {
      const registry = yield* BoardRegistry;
      ...
      yield* committer.reconcileChunk("b-orphan" as never, lanes, [
        { _tag: "missing", item: b, ticketId: mappedB?.ticketId as string, confirmedDeleted: true },
      ]);
  ...
  it.effect("post-tx pipeline start: a create into an auto lane starts the pipeline after the chunk", () =>
    Effect.gen(function* () {
      const registry = yield* BoardRegistry;
      ...
      const autoLanes: ReconcileLanes = { destinationLane: "work" as never, closedLane: "done" as never };
...
it.effect("Fix 2: a later failing delta rolls back the close WITHOUT cancelling the provider mid-tx", () =>
  Effect.gen(function* () {
    const providerCalls = yield* Ref.make<ReadonlyArray<ProviderCall>>([]);
    const published = yield* Ref.make<ReadonlyArray<string>>([]);
    yield* Effect.gen(function* () {
...
it.effect("Fix 3: a UNIQUE violation on the mapping insert rolls back the chunk; no orphan ticket", () =>
  Effect.gen(function* () {
    const providerCalls = yield* Ref.make<ReadonlyArray<ProviderCall>>([]);
    const published = yield* Ref.make<ReadonlyArray<string>>([]);
    yield* Effect.gen(function* () {
...
it.effect("Fix 6: created / changed / closed tickets are published to WorkflowBoardEvents post-tx", () =>
  Effect.gen(function* () {
    const providerCalls = yield* Ref.make<ReadonlyArray<ProviderCall>>([]);
    const published = yield* Ref.make<ReadonlyArray<string>>([]);
    yield* Effect.gen(function* () {
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        Effect.provide(
          makeCommitterLayer(providerCalls, published, failingRecoverEngineLayer),
        ),
      );
    }),
);
