...
const unimplemented = (name: string) => () =>
  Effect.fail(
    new GitHubCliCommandError({
      command: "gh",
      cwd: "/repo",
      cause: new Error(`unexpected ${name}`),
    }),
  );
...
describe("GitHubPortLive", () => {
  ...
  describe("findPrForBranch", () => {
    it.effect("returns the first open PR for the head selector", () => {
      ...
      return Effect.gen(function* () {
        const port = yield* GitHubPort;
        const result = yield* port.findPrForBranch({
          cwd: "/repo",
          branch: "workflow/ticket-x",
        });
        assert.deepStrictEqual(result, {
          number: 7,
          url: "https://github.com/o/r/pull/7",
  ...
  describe("failingCheckLogs", () => {
    it.effect("returns null when no checks fail", () =>
      Effect.gen(function* () {
        const port = yield* GitHubPort;
        const result = yield* port.failingCheckLogs({
          cwd: "/repo",
          prNumber: 7,
        });
        assert.equal(result, null);
        ...
        Effect.provide(
          GitHubPortLive.pipe(
            Layer.provide(
              ghLayer({
                listPullRequestChecks: () => Effect.succeed([check({ bucket: "pass" })]),
              }),
            ),
            Layer.provide(registryLayer(null)),
            Layer.provide(gitLayer(() => gitResult(), [])),
