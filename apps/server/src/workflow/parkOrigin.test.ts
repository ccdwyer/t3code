...
describe("buildParkOrigin / parseParkOrigin", () => {
  it("round-trips a step origin (stepKey + key)", () => {
    const t = target();
    const json = buildParkOrigin({ src: "step", target: t, stepKey: "code", key: "failure" });
  ...
  it("round-trips a lane_on origin (key only)", () => {
    ...
    const parsed = parseParkOrigin(json);
    assert.deepEqual(parsed, { src: "lane_on", key: "success", fp: parkTargetFingerprint(t) });
  });
  ...
  it("round-trips a transition origin (no positional index, fp is identity)", () => {
    ...
    const parsed = parseParkOrigin(json);
    assert.deepEqual(parsed, { src: "transition", fp: parkTargetFingerprint(t) });
    assert.notInclude(json, "index");
  });

  it("round-trips an event origin (name)", () => {
    const t = target();
    const json = buildParkOrigin({ src: "event", target: t, name: "ci.failed" });
    const parsed = parseParkOrigin(json);
    assert.deepEqual(parsed, { src: "event", name: "ci.failed", fp: parkTargetFingerprint(t) });
  });
