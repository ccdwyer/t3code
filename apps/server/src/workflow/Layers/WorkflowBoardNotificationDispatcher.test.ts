...
const buildLayer = (
  ...
  byTicket: Record<string, TicketDetail | null>,
) =>
    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        buildLayer(recorder, {
          ...
          "ticket-2": detail(
            makeTicketRow({ ticketId: "ticket-2", status: "running" }),
          ),
          "ticket-3": null,
        }),
      ),
  ...
  it.effect("falls back to a non-empty title when the ticket title is blank", () => {
    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        buildLayer(recorder, {
          "ticket-8": detail(
            makeTicketRow({ ticketId: "ticket-8", title: "   " }),
          ),
        }),
      ),
    );
  ...
  it.effect("redacts and caps the body, and falls back when reason is empty", () => {
    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        buildLayer(recorder, {
          "ticket-6": detail(
            makeTicketRow({ ticketId: "ticket-6", attentionReason: longReason }),
          ),
          "ticket-7": detail(
            makeTicketRow({ ticketId: "ticket-7", attentionReason: "" }),
          ),
        }),
      ),
    );
