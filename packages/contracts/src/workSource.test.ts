...
describe("workSource contracts", () => {
  it("defaults github selector state to 'all'", () => {
    const sel = Schema.decodeUnknownSync(GithubSelector)({
      owner: "o",
      repo: "r",
    });
  ...
  it("ImportWorkItemsResult decodes an empty result", () => {
    const decoded = Schema.decodeUnknownSync(ImportWorkItemsResult)({
      imported: [],
      skipped: [],
    });
...
describe("compileAutoPullRule", () => {
  ...
  it("assigned-to-anyone → bare var; specific → in; empty → ALWAYS", () => {
    assert.deepEqual(compileAutoPullRule({ assignee: { kind: "anyone" } }), {
      var: "assignees",
    });
...
describe("decodeAutoPullRule round-trips compiled criteria", () => {
  for (const c of [
    { labels: { mode: "any", values: ["XS"] } },
    { labels: { mode: "all", values: ["A", "B"] } },
    { assignee: { kind: "anyone" } },
    { assignee: { kind: "login", value: "x" } },
    { state: "open" },
    { labels: { mode: "any", values: ["XS", "S"] }, state: "closed" },
    {},
  ] as AutoPullCriteria[]) {
    it(`round-trips ${JSON.stringify(c)}`, () =>
      assert.deepEqual(decodeAutoPullRule(compileAutoPullRule(c)), c));
  }
