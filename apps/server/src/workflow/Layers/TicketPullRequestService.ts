...
const make = Effect.gen(function* () {
  ...
  const open: TicketPullRequestServiceShape["open"] = (input) =>
    Effect.gen(function* () {
      ...
      const base =
        input.step.base ?? (yield* github.defaultBranch(input.worktreePath));

      // 4. Render the PR title and body, always appending the ticket trailer.
      const title =
        input.step.titleTemplate !== undefined
          ? applyInstructionTemplate(input.step.titleTemplate, vars)
          : ticketTitle;
