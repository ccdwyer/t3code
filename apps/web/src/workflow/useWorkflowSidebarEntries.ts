...
export type ProjectBoardsQueryResult =
  ...
  | { readonly status: "error"; readonly error: string; readonly revalidating: boolean }
  | {
...
function readProjectBoardsResult(
  ...
  if (AsyncResult.isSuccess(result)) {
    return { status: "success", entries: result.value, revalidating: result.waiting };
  }
  if (result._tag === "Failure") {
    return { status: "error", error: formatQueryError(result.cause), revalidating: result.waiting };
  }
