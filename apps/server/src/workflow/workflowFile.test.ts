        ...
        base([
          {
            ...
            on: {
              failure: {
                park: "issue",
                actions: [{ label: "Retry", to: "planning" }],
              },
            },
          },
        ]),
            ...
            transitions: [
              {
                ...
                to: {
                  park: "waiting",
                  actions: [{ label: "Continue", to: "backlog" }],
                },
              },
            ],
            onEvent: [
              {
                ...
                to: {
                  park: "issue",
                  actions: [{ label: "Investigate", to: "backlog" }],
                },
              },
            ],
            // lane.on
...
describe("lintWorkflowDefinition source autoPull rules", () => {
  ...
  it("source autoPull: disallowed operator → invalid_json_logic", () => {
    const errors = lintWorkflowDefinition(
      defWithSource({
        autoPull: { rule: { some: [{ var: "labels" }, true] } },
      }),
      ctx,
    );
  ...
  it("source autoPull: valid labels/state rule passes", () => {
    const errors = lintWorkflowDefinition(
      defWithSource({
        autoPull: {
          rule: {
            and: [{ in: ["XS", { var: "labels" }] }, { "==": [{ var: "state" }, "open"] }],
          },
        },
      }),
      ctx,
  ...
  it("source autoPull: two disallowed operators → two invalid_json_logic errors", () => {
    ...
    const errors = lintWorkflowDefinition(
      defWithSource({
        autoPull: {
          rule: {
            and: [{ some: [{ var: "labels" }, true] }, { all: [{ var: "labels" }, true] }],
          },
        },
      }),
      ctx,
          ...
          pipeline: [
            {
              ...
              retry: {
                maxAttempts: 2,
                escalate: { instance: "opencode_main" },
              },
            },
          ],
          on: { success: "done" },
