...
const pullRequestServiceCalls: Array<{
    ...
    RealStepExecutorLive.pipe(
      ...
      Layer.provideMerge(
        Layer.succeed(ProviderDispatchOutbox, {
          ...
          ackSteerDelivered: () => Effect.succeed(false),
          listStagedSteerDeliveries: () => Effect.succeed([]),
          clearStagedSteerDelivery: () => Effect.void,
    ...
    yield* Effect.forEach(messages, (message, index) => {
      ...
      return sql`
        ...
        VALUES (
          ...
          ${message.author},
          ${message.body},
          ${encodeUnknownJsonString(attachments)},
...
mk({ ok: true, turnId: "turn-stub" as never })("RealStepExecutor success", (it) => {
  ...
  it.effect("appends the ticket discussion to the dispatched instruction", () =>
    Effect.gen(function* () {
      ...
      yield* seedTicketMessages(discussionContext, [
        {
          author: "user",
          body: "Use the existing retry helper",
          attachments: 0,
        },
  ...
  const seedPackFor = (ctx: StepExecutionContext, body: string) =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      yield* sql`
        ...
        VALUES (
          ${ctx.ticketId as string}, ${ctx.laneKey as string}, 'implement',
          '2026-07-25T00:00:00.000Z', NULL,
          ${encodeUnknownJsonString([{ key: "notes", body, autoGenerated: true }])}
        )
        ON CONFLICT (ticket_id, for_lane) DO UPDATE SET sections_json = excluded.sections_json
      `;
    });

  it.effect("appends the handoff pack on the lane's first agent step", () =>
    Effect.gen(function* () {
      dispatchStartInputs.length = 0;
      const executor = yield* StepExecutor;
      const ctx = {
        ...context,
        ticketId: "ticket-pack-append" as never,
        isFirstAgentStep: true,
      } satisfies StepExecutionContext;
      yield* seedStepStartedFor(ctx, "event-step-started-pack-append");
      yield* seedPackFor(ctx, "the previous lane refactored the parser");
  ...
  it.effect("does NOT append the pack on a later agent step in the same lane", () =>
    Effect.gen(function* () {
      dispatchStartInputs.length = 0;
      const executor = yield* StepExecutor;
      const ctx = {
        ...context,
        ticketId: "ticket-pack-later" as never,
        isFirstAgentStep: false,
      } satisfies StepExecutionContext;
      yield* seedStepStartedFor(ctx, "event-step-started-pack-later");
      yield* seedPackFor(ctx, "should not be repeated");

      yield* executor.execute(ctx);
      const dispatched = dispatchStartInputs[0] as {
        readonly instruction: string;
      };
      assert.notInclude(dispatched.instruction, "## Handoff context from lane");
    }),
  );

  it.effect("fills an explicit placeholder on any step, and only the first one", () =>
    Effect.gen(function* () {
      dispatchStartInputs.length = 0;
      const executor = yield* StepExecutor;
      const ctx = {
        ...context,
        ticketId: "ticket-pack-placeholder" as never,
        // Not the first agent step: an explicit placeholder still works.
        isFirstAgentStep: false,
        step: {
          ...context.step,
          instruction: "Before {{ticket.contextPack}} after {{ticket.contextPack}} end",
        },
      } as StepExecutionContext;
      yield* seedStepStartedFor(ctx, "event-step-started-pack-placeholder");
      yield* seedPackFor(ctx, "spliced body");

      yield* executor.execute(ctx);
      const dispatched = dispatchStartInputs[0] as {
        readonly instruction: string;
      };
      assert.include(dispatched.instruction, "spliced body");
      // Single-use: the second reference renders as the empty marker.
      assert.include(dispatched.instruction, "(no handoff context)");
      assert.equal(dispatched.instruction.split("spliced body").length - 1, 1);
      assert.notInclude(dispatched.instruction, "{{ticket.contextPack}}");
      // No pack was appended on top of the explicit placement.
      assert.equal(dispatched.instruction.split("## Handoff context from lane").length - 1, 1);
    }),
  );

  it.effect("renders the empty marker when the placeholder is used with no pack", () =>
    Effect.gen(function* () {
      dispatchStartInputs.length = 0;
      const executor = yield* StepExecutor;
      const ctx = {
        ...context,
        ticketId: "ticket-pack-absent" as never,
        isFirstAgentStep: true,
        step: {
          ...context.step,
          instruction: "Context: {{ticket.contextPack}}",
        },
      } as StepExecutionContext;
      yield* seedStepStartedFor(ctx, "event-step-started-pack-absent");
  ...
  it.effect("never re-expands placeholder text that appears inside the pack body", () =>
    Effect.gen(function* () {
      dispatchStartInputs.length = 0;
      const executor = yield* StepExecutor;
      const ctx = {
        ...context,
        ticketId: "ticket-pack-reexpand" as never,
        isFirstAgentStep: true,
      } satisfies StepExecutionContext;
      yield* seedStepStartedFor(ctx, "event-step-started-pack-reexpand");
      // A captured output can legitimately contain this text; splicing the pack
      // after templating is what stops it being treated as a placeholder.
      yield* seedPackFor(ctx, "the agent wrote {{ticket.description}} in its notes");

      yield* executor.execute(ctx);
      const dispatched = dispatchStartInputs[0] as {
        readonly instruction: string;
      };
      assert.include(dispatched.instruction, "{{ticket.description}}");
    }),
  );
  ...
  it.effect("runs a trusted script step through the shared prepared worktree path", () =>
    Effect.gen(function* () {
      ...
      const executor = yield* StepExecutor;
      const sql = yield* SqlClient.SqlClient;
      yield* fileSystem.makeDirectory("/tmp/wt-ticket-1", {
        recursive: true,
      });
      yield* seedBoard;
  ...
  it.effect("reads normal instruction files that resolve inside the repo root", () =>
    Effect.gen(function* () {
      ...
      const fileSystem = yield* FileSystem.FileSystem;
      const executor = yield* StepExecutor;
      yield* fileSystem.makeDirectory(`${repoRoot}/instructions`, {
        recursive: true,
      });
...
mk({ ok: true, turnId: "turn-stub" as never })("RealStepExecutor handoff", (it) => {
  ...
  it.effect("spills over-budget handoff output to a per-ticket scratch file + path ref", () =>
    Effect.gen(function* () {
      ...
      yield* fileSystem
        .remove("/tmp/wt-ticket-1/.t3/ticket/ticket-handoff-spill", {
          recursive: true,
        })
        .pipe(Effect.catch(() => Effect.void));
    }),
...
mk(
  ...
  it.effect("preserves handoff-like syntax inside an inlined description verbatim", () =>
    Effect.gen(function* () {
      ...
      const ctx = {
        ...descSpillContext,
        ticketId: "ticket-desc-handoff-syntax" as never,
      };
  ...
  it.effect("uses the 120k budget when no maxInputChars is declared (today's behavior)", () =>
    Effect.gen(function* () {
      ...
      const ctx = {
        ...descSpillContext,
        ticketId: "ticket-desc-120k" as never,
      };
...
mk(
  ...
  it.effect("spills a long description to DESCRIPTION.md and replaces all occurrences", () =>
    Effect.gen(function* () {
      ...
      yield* fileSystem
        .remove("/tmp/wt-ticket-1/.t3/ticket/ticket-desc-spill", {
          recursive: true,
        })
        .pipe(Effect.catch(() => Effect.void));
    }),
  ...
  it.effect("spills both the description and an over-budget handoff, staying within budget", () =>
    Effect.gen(function* () {
      ...
      yield* fileSystem
        .remove("/tmp/wt-ticket-1/.t3/ticket/ticket-desc-spill-handoff", {
          recursive: true,
        })
        .pipe(Effect.catch(() => Effect.void));
    }),
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.isAtMost(dispatched.instruction.length, 800);
        yield* fileSystem
          .remove("/tmp/wt-ticket-1/.t3/ticket/ticket-desc-fallback", {
            recursive: true,
          })
          .pipe(Effect.catch(() => Effect.void));
      }),
...
mk(
  ...
  it.effect("inlines a description shorter than the pointer even when over budget", () =>
    Effect.gen(function* () {
      ...
      const ctx = {
        ...descSpillContext,
        ticketId: "ticket-desc-guard" as never,
      };
...
const continueContext: StepExecutionContext = {
  ...
  step: {
    ...
    type: "agent",
    agent: {
      instance: continueAgent.instance as never,
      model: continueAgent.model as never,
    },
    instruction: "Implement the ticket",
