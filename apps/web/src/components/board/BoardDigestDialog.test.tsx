...
describe("digestAttentionLabel", () => {
  ...
  it("labels a parked-waiting ticket as 'waiting'", () => {
    expect(digestAttentionLabel({ status: "parked", attentionKind: "parked_waiting" })).toBe(
      "waiting",
    );
  });
  ...
  it("labels a waiting-on-user ticket as 'waiting'", () => {
    expect(
      digestAttentionLabel({ status: "waiting_on_user", attentionKind: "waiting_for_approval" }),
    ).toBe("waiting");
  });
