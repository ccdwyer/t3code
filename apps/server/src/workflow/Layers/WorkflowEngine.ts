...
const make = Effect.gen(function* () {
  ...
  interface EnterLaneCoreOptions {
    ...
    readonly serialize?: (<A, E, R>(body: Effect.Effect<A, E, R>) => Effect.Effect<A, E, R>) | undefined;
  ...
  const enterLaneCore = (
    ...
    { readonly starts: ReadonlyArray<PipelineStartAction>; readonly acted: "moved" | "queued" | "none" },
  ...
  const cancellableProviderTurnsForTicket: WorkflowEngineShape["cancellableProviderTurnsForTicket"] =
    (ticketId) =>
      cancellableProviderDispatchesForTicket(ticketId).pipe(
        Effect.map((rows) =>
          rows.map((row) => ({ threadId: row.threadId, turnId: row.turnId })),
        ),
      );
  ...
  const recoverBoardWip: WorkflowEngineShape["recoverBoardWip"] = (boardId) =>
    Effect.gen(function* () {
      ...
      for (const lane of definition.lanes) {
        yield* withAdmissionLock(
          boardId,
          Effect.uninterruptible(admitNext(boardId, lane.key)),
        );
      }
