...
const blockedExecutor = makeScriptedExecutor(() => ({
  _tag: "blocked",
  reason: "no trust",
}));
...
const recoveredCancelExecutor = makeScriptedExecutor(() => ({
  _tag: "completed",
}));
...
const reviewLoopExecutor = makeScriptedExecutor((call) => {
  ...
  return {
    _tag: "completed",
    output: { verdict: call < 4 ? "revise" : "approve" },
  };
});
