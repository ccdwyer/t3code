...
layer("WorkflowEventCommitter", (it) => {
  ...
  it.effect(
    "writes exactly one outbox row when an event flips a ticket into waiting_on_user",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("does not write a second outbox row when the ticket stays in the same needs-you status", () =>
    Effect.gen(function* () {
