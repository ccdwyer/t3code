...
const makeWorktree = Effect.gen(function* () {
  ...
  const root = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3-scratch-wt-" });
...
const scratch = (root: string, relativePath: string, mime: string) =>
  ({ _tag: "ticket-scratch", workspaceRoot: root, ticketId: TICKET, relativePath, mime }) as const;
