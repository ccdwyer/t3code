...
layer("WorkflowProjectionPipeline", (it) => {
  ...
  it.effect("context pack: compiled upserts, lane entry clears, edited replaces and deletes", () =>
    Effect.gen(function* () {
      const pipeline = yield* WorkflowProjectionPipeline;
      const sql = yield* SqlClient.SqlClient;
      const packs = () =>
        sql<{
          ...
          SELECT
            for_lane AS "forLane",
            from_lane AS "fromLane",
            compiled_at AS "compiledAt",
            edited_at AS "editedAt",
            sections_json AS "sectionsJson"
          FROM projection_context_pack
          WHERE ticket_id = 'pack-t'
        `;

      yield* pipeline.projectEvent({
        type: "TicketCreated",
        eventId: "p0" as never,
        ticketId: "pack-t" as never,
        streamVersion: 0,
        occurredAt: "2026-07-25T00:00:00.000Z" as never,
        payload: { boardId: "b-1" as never, title: "Pack" as never, laneKey: "backlog" as never },
      });

      // Routed batch order: the move lands first (clearing), then the pack event.
      yield* pipeline.projectEvent({
        type: "TicketMovedToLane",
        eventId: "p1" as never,
        ticketId: "pack-t" as never,
        ...
        payload: { toLane: "review" as never, laneEntryToken: "tok" as never, reason: "routed" },
      });
      yield* pipeline.projectEvent({
        ...
        ticketId: "pack-t" as never,
        streamVersion: 2,
        occurredAt: "2026-07-25T00:00:02.000Z" as never,
        payload: {
          forLane: "review" as never,
          fromLane: "implement" as never,
          sections: [{ key: "notes", body: "compiled", autoGenerated: true }],
        },
      });
      let rows = yield* packs();
      assert.lengthOf(rows, 1);
      assert.strictEqual(rows[0]?.fromLane, "implement");
      assert.strictEqual(rows[0]?.editedAt, null);

      // An edit replaces sections and stamps editedAt from occurredAt, while
      // preserving from_lane / compiled_at.
      yield* pipeline.projectEvent({
        type: "TicketContextPackEdited",
        eventId: "p3" as never,
        ticketId: "pack-t" as never,
        streamVersion: 3,
        occurredAt: "2026-07-25T00:00:03.000Z" as never,
        payload: {
          forLane: "review" as never,
          sections: [{ key: "notes", body: "edited", autoGenerated: false }],
        },
      });
      rows = yield* packs();
      assert.include(rows[0]?.sectionsJson ?? "", "edited");
      assert.strictEqual(rows[0]?.editedAt, "2026-07-25T00:00:03.000Z");
      assert.strictEqual(rows[0]?.fromLane, "implement");
      assert.strictEqual(rows[0]?.compiledAt, "2026-07-25T00:00:02.000Z");

      // A MANUAL re-entry into the same lane clears the pack: no stale pack may
      // survive an entry that carries no compiled event behind it.
      yield* pipeline.projectEvent({
        type: "TicketMovedToLane",
        eventId: "p4" as never,
        ticketId: "pack-t" as never,
        streamVersion: 4,
        occurredAt: "2026-07-25T00:00:04.000Z" as never,
        payload: { toLane: "review" as never, laneEntryToken: "tok2" as never, reason: "manual" },
      });
      assert.lengthOf(yield* packs(), 0);

      // TicketQueued clears too — its payload key is `lane`, not `toLane`.
      yield* pipeline.projectEvent({
        type: "TicketContextPackCompiled",
        eventId: "p5" as never,
        ticketId: "pack-t" as never,
        streamVersion: 5,
        occurredAt: "2026-07-25T00:00:05.000Z" as never,
        payload: {
          forLane: "review" as never,
          fromLane: "implement" as never,
          sections: [{ key: "notes", body: "again", autoGenerated: true }],
        },
      });
      assert.lengthOf(yield* packs(), 1);
      yield* pipeline.projectEvent({
        type: "TicketQueued",
        eventId: "p6" as never,
        ticketId: "pack-t" as never,
        streamVersion: 6,
        occurredAt: "2026-07-25T00:00:06.000Z" as never,
        payload: { lane: "review" as never },
      });
      assert.lengthOf(yield* packs(), 0);

      // TicketRouted is a lane entry as well, so it clears the destination's pack.
      yield* pipeline.projectEvent({
        type: "TicketContextPackCompiled",
        eventId: "p6b" as never,
        ticketId: "pack-t" as never,
        streamVersion: 6,
        occurredAt: "2026-07-25T00:00:06.500Z" as never,
        payload: {
          forLane: "review" as never,
          fromLane: "implement" as never,
          sections: [{ key: "notes", body: "third", autoGenerated: true }],
        },
      });
      assert.lengthOf(yield* packs(), 1);
      yield* pipeline.projectEvent({
        type: "TicketRouted",
        eventId: "p6c" as never,
        ticketId: "pack-t" as never,
        streamVersion: 6,
        occurredAt: "2026-07-25T00:00:06.750Z" as never,
        payload: { fromLane: "implement" as never, toLane: "review" as never },
      });
      assert.lengthOf(yield* packs(), 0);

      // Edited against a missing row is a defensive no-op, not a projection crash.
      yield* pipeline.projectEvent({
        type: "TicketContextPackEdited",
        eventId: "p7" as never,
        ticketId: "pack-t" as never,
        streamVersion: 7,
        occurredAt: "2026-07-25T00:00:07.000Z" as never,
        payload: { forLane: "review" as never, sections: [] },
      });
      assert.lengthOf(yield* packs(), 0);
    }),
  );
