...
export function TicketActionSheetScreen(props: TicketActionSheetScreenProps) {
  // Deep links may omit params; guard instead of trusting the static type.
  const params: Partial<TicketActionSheetScreenProps["route"]["params"]> = props.route.params ?? {};

  const environmentIdRaw = params.environmentId ?? null;
  const boardIdRaw = params.boardId ?? null;
  const ticketIdRaw = params.ticketId ?? null;
