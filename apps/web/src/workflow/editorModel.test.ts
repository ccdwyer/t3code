      ...
      lanes: baseDefinition.lanes.map((lane) =>
        lane.key === "run"
          ? {
              ...
              transitions: [
                {
                  when: { "==": [{ var: "ticket.status" }, "queued"] },
                  to: "queue",
                },
                {
                  when: { "==": [{ var: "ticket.status" }, "done"] },
                  to: "done",
                },
                {
                  when: { "==": [{ var: "ticket.status" }, "retry"] },
                  to: "queue",
                },
              ],
            }
...
describe("park-target editing", () => {
  ...
  it("select-value helpers map parks to distinct sentinels", () => {
    ...
    expect(parseRouteSelectValue("done")).toEqual({
      kind: "lane",
      laneKey: "done",
    });
    expect(parseRouteSelectValue("__park_issue")).toEqual({
      kind: "park",
      substate: "issue",
    });
    expect(parseRouteSelectValue("__park_waiting")).toEqual({
      kind: "park",
      substate: "waiting",
    });
  });
  ...
  it("preserves the park object when a sibling transition field (when) is edited", () => {
    ...
    expect(laneOf(model, "run")?.transitions?.[0]?.when).toEqual({
      "==": [{ var: "x" }, 1],
    });
  ...
  it("toggles substate while preserving label and actions, and swaps park↔lane", () => {
    ...
    if (isParkRouteTarget(toggled)) {
      ...
      expect(toggled.actions[0]).toEqual({
        label: "Try again",
        to: "run",
        hint: "rerun the lane",
      });
  ...
  it.effect("round-trips a park built via the mutators through the schema decode", () =>
    Effect.gen(function* () {
      ...
      model = addParkAction(model, path);
      model = updateParkAction(model, path, 1, {
        label: "Send back",
        to: "queue",
      });
