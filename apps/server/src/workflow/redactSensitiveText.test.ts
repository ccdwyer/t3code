...
describe("redactSensitiveText", () => {
  describe("GitHub tokens", () => {
    ...
    it("redacts gho_ tokens via the gh prefix pattern (not high-entropy)", () => {
      ...
      assert.equal(
        redactSensitiveText("auth gho_abcdefghij1234567890ab"),
        "auth [redacted]",
      );
    });
  ...
  describe("AWS keys", () => {
    it("redacts AKIA keys", () => {
      assert.equal(
        redactSensitiveText("key: AKIAIOSFODNN7EXAMPLE"),
        "key: [redacted]",
      );
    });
  ...
  describe("NAME=value / NAME: value lines", () => {
    it("redacts TOKEN= assignments", () => {
      assert.equal(
        redactSensitiveText("MY_TOKEN=supersecretvalue123"),
        "MY_TOKEN=[redacted]",
      );
    });
    ...
    it("redacts SECRET: assignments", () => {
      assert.equal(
        redactSensitiveText("APP_SECRET: mysecretvalue"),
        "APP_SECRET=[redacted]",
      );
    });
    ...
    it("redacts PASSWORD= assignments", () => {
      assert.equal(
        redactSensitiveText("DB_PASSWORD=hunter2"),
    ...
    it("redacts KEY= assignments", () => {
      assert.equal(
        redactSensitiveText("API_KEY=abcdef123456"),
        "API_KEY=[redacted]",
      );
    });
    ...
    it("redacts case-insensitively (lowercase token)", () => {
      assert.equal(
        redactSensitiveText("access_token=abc123xyz"),
