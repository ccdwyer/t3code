...
const make = Effect.gen(function* () {
  ...
  // The four section keys the contract allows. A corrupt row carrying anything
  // else must degrade to "no pack" here; letting it through only moves the failure
  // to the wire encoder, which would take down the whole ticket-detail read.
  const CONTEXT_PACK_SECTION_KEYS = new Set([
    "prior_outputs",
    "diff_summary",
    "failed_attempts",
    "notes",
  ...
  const getContextPack: WorkflowReadModelShape["getContextPack"] = (ticketId, laneKey) =>
    wrap(
      sql<ContextPackSqlRow>`
        SELECT
          for_lane AS "forLane",
        ...
        WHERE ticket_id = ${ticketId} AND for_lane = ${laneKey}
        LIMIT 1
      `,
    ).pipe(
      Effect.map((rows) => {
        ...
        // sections_json is written only by the projection from decoded events, so a
        // parse failure means corruption rather than untrusted input: degrade to no
        // pack instead of failing the read that carries it.
        let sections: ReadonlyArray<WorkflowContextPackSection>;
        try {
          const parsed: unknown = JSON.parse(row.sectionsJson);
          // Shape-check before trusting it: a corrupt row must not reach the wire
          // encoder as a non-array (or as sections missing required fields).
          if (
            !Array.isArray(parsed) ||
            !parsed.every(
              (entry): entry is WorkflowContextPackSection =>
                typeof entry === "object" &&
                entry !== null &&
                CONTEXT_PACK_SECTION_KEYS.has((entry as { key?: unknown }).key as string) &&
                typeof (entry as { body?: unknown }).body === "string" &&
                typeof (entry as { autoGenerated?: unknown }).autoGenerated === "boolean",
            )
          ) {
            return null;
          }
        ...
        return {
          forLane: row.forLane as LaneKey,
          fromLane: row.fromLane as LaneKey,
          compiledAt: row.compiledAt as WorkflowContextPackView["compiledAt"],
          ...(row.editedAt === null
            ? {}
            : {
                editedAt: row.editedAt as WorkflowContextPackView["editedAt"],
              }),
          sections,
        } satisfies WorkflowContextPackView;
      }),
    );
  ...
  const getTicketDetail: WorkflowReadModelShape["getTicketDetail"] = (ticketId) =>
    Effect.gen(function* () {
      ...
      return {
        ...
        steps,
        messages,
        ...(syncedSource !== undefined ? { syncedSource } : {}),
      };
    });
