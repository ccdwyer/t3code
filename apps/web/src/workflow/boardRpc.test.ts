...
describe("boardRpc", () => {
  it("delegates listBoards and createBoard through the workflow EnvironmentApi", async () => {
    ...
    expect(api.workflow.createBoard).toHaveBeenCalledWith({
      projectId,
      name: "Delivery",
      agent,
    });
    expect(api.workflow.deleteBoard).toHaveBeenCalledWith({ boardId });
    expect(api.workflow.renameBoard).toHaveBeenCalledWith({
      boardId,
    ...
    expect(api.workflow.deleteTicket).toHaveBeenCalledWith({
      ticketId: TicketId.make("ticket-1"),
    });
