...
describe("resolveParkActions", () => {
  ...
  it("resolves a step origin by fingerprint", () => {
          ...
          pipeline: [
            {
              ...
              instruction: "do it",
              on: { failure: issuePark("Broke", [{ label: "Retry", to: "impl" }]) },
            },
          ],
        },
      ],
    ...
    const origin = buildParkOrigin({ src: "step", target, stepKey: "code", key: "failure" });
      ...
      lanes: [
        {
          ...
          name: "Impl",
          entry: "auto",
          on: { failure: issuePark("Hit a snag", [{ label: "Retry", to: "backlog" }]) },
        },
      ],
    });
  ...
  it("returns null when the routing site became a bare lane key", () => {
    ...
    const asLaneMove = decode({
      name: "wf",
      lanes: [
        { key: "impl", name: "Impl", entry: "auto", on: { failure: "backlog" } },
      ...
      lanes: [
        {
          ...
          name: "Impl",
          entry: "auto",
          onEvent: [{ name: "pr_closed", to: issuePark("X", [{ label: "Y", to: "impl" }]) }],
        },
      ],
    });
    const origin = buildParkOrigin({
      ...
      lanes: [
        {
          ...
          name: "Impl",
          entry: "auto",
          onEvent: [{ name: "pr_reopened", to: issuePark("X", [{ label: "Y", to: "impl" }]) }],
        },
      ],
    });
