...
describe("GithubIssuesProvider", () => {
  describe("listPage", () => {
    ...
    it.effect("Fix 6: 200 body that is not an array → WorkSourceTransientError (not a defect)", () => {
      const { testLayer } = makeTestLayer({ responseBody: { message: "garbage" }, responseStatus: 200 });
  ...
  describe("getItem", () => {
    ...
    it.effect("401 → WorkSourceAuthError (typed failure, NOT null)", () => {
      const { testLayer } = makeTestLayer({ responseBody: { message: "Bad credentials" }, responseStatus: 401 });
