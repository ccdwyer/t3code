...
const make = Effect.gen(function* () {
  ...
  const bestEffort = <A, E>(label: string, effect: Effect.Effect<A, E>) =>
    effect.pipe(
      Effect.catchCause((cause) =>
        Effect.logWarning("workflow worktree cleanup step failed", {
          label,
          cause,
        }),
      ),
  ...
  const collectBoardPlan: WorkflowWorktreeJanitorShape["collectBoardPlan"] = (boardId) =>
    Effect.gen(function* () {
      ...
      Effect.catchCause((cause) =>
        Effect.logWarning("workflow worktree cleanup board plan failed", {
          boardId,
          cause,
        }).pipe(Effect.as(null)),
      ),
  ...
  const collectTicketPlan: WorkflowWorktreeJanitorShape["collectTicketPlan"] = (ticketId) =>
    Effect.gen(function* () {
      ...
      Effect.catchCause((cause) =>
        Effect.logWarning("workflow worktree cleanup ticket plan failed", {
          ticketId,
          cause,
        }).pipe(Effect.as(null)),
      ),
  ...
  return {
    ...
    collectTicketPlan,
    run,
  } satisfies WorkflowWorktreeJanitorShape;
});
