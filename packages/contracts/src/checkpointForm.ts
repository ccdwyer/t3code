...
export const validateCheckpointSubmission = (
  ...
  if (decision !== undefined) {
    if (submission.decision === undefined) {
      return {
        ok: false,
        message: "a decision is required for this checkpoint",
      };
    }
  ...
  for (const field of form.fields) {
    ...
    if (field.kind === "select") {
      ...
      if (typeof raw !== "string") {
        return {
          ok: false,
          message: `answer for "${field.key}" must be a single option`,
        };
      }
      ...
      if (field.allowOther !== true && !field.options.some((option) => option.value === raw)) {
        return {
          ok: false,
          message: `"${raw}" is not an option for "${field.label}"`,
        };
      }
      ...
      if (raw.length > CHECKPOINT_ANSWER_MAX_TEXT) {
        return {
          ok: false,
          message: `answer for "${field.label}" is too long`,
        };
      }
      ...
      if (field.allowOther === true && raw === CHECKPOINT_OTHER_SENTINEL) {
        return {
          ok: false,
          message: `"${field.label}" needs an actual answer`,
        };
      }
    ...
    if (checked === null) {
      return {
        ok: false,
        message: `answer for "${field.key}" must be a list of checked items`,
      };
    }
    if (checked.length > CHECKPOINT_ANSWER_MAX_ITEMS) {
      return {
        ok: false,
        message: `too many items checked for "${field.label}"`,
      };
    }
    ...
    if (unknown !== undefined) {
      return {
        ok: false,
        message: `"${unknown}" is not an item of "${field.label}"`,
      };
    }
    ...
    if (outcome === "success") {
      if (field.requireAll === true && unique.length !== field.items.length) {
        return {
          ok: false,
          message: `every item of "${field.label}" must be checked`,
        };
      }
      if (field.required === true && unique.length === 0) {
        return {
          ok: false,
          message: `at least one item of "${field.label}" must be checked`,
        };
      }
    }
    if (unique.length > 0) {
