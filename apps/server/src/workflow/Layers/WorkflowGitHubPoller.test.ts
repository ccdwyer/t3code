      ...
      prDetail: ({ prNumber }) =>
        Effect.suspend(() => {
          ...
          if (script === undefined || script.failPrDetail) {
            return Effect.fail(
              new WorkflowEventStoreError({
                message: `gh prDetail failed for #${prNumber}`,
              }),
            );
          }
          ...
          if (next === undefined) {
            return Effect.fail(
              new WorkflowEventStoreError({
                message: `no scripted detail for #${prNumber}`,
              }),
            );
          }
      ...
      postTicketMessage: (input) =>
        Effect.suspend(() =>
          postAlwaysFails
            ? Effect.fail(
                new WorkflowEventStoreError({
                  message: "poison-pill post failure",
                }),
              )
      ...
      ingestExternalEvent: (input) =>
        Effect.suspend(() => {
          if (ingestAlwaysFails) {
            return Effect.fail(
              new WorkflowEventStoreError({
                message: "poison-pill ingest failure",
              }),
            );
          }
          if (ingestFailureCount > 0) {
            ...
            return Effect.fail(
              new WorkflowEventStoreError({
                message: "transient ingest failure",
              }),
            );
          }
...
layer("WorkflowGitHubPoller", (it) => {
  ...
  it.effect("9. two tickets on one board both merge -> distinct lifecycle obs + both ingest", () =>
    Effect.gen(function* () {
      ...
      scriptPr(91, {
        details: [detail({ number: 91, headSha: "sha1", state: "merged" })],
      });
      scriptPr(92, {
        details: [detail({ number: 92, headSha: "sha1", state: "merged" })],
      });
