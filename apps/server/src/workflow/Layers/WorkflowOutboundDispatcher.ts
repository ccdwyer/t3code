...
const permanent = (message: string): DeliveryFailure => ({
  message,
  retryable: false,
});
  ...
  Effect.gen(function* () {
    ...
    const sweep: WorkflowOutboundDispatcherShape["sweep"] = () =>
      Effect.gen(function* () {
        ...
        `.pipe(
          Effect.catchCause((cause) =>
            Effect.logWarning("workflow.outbound.select-failed", {
              cause,
            }).pipe(Effect.as([] as ReadonlyArray<DeliveryRow>)),
          ),
        );
    ...
    const recoverStaleClaims: WorkflowOutboundDispatcherShape["recoverStaleClaims"] = () =>
      ...
      `.pipe(
        Effect.asVoid,
        Effect.catchCause((cause) =>
          Effect.logWarning("workflow.outbound.recover-stale-claims-failed", {
            cause,
          }),
        ),
      );
    ...
    return {
      ...
      recoverStaleClaims,
      start,
    } satisfies WorkflowOutboundDispatcherShape;
  });
