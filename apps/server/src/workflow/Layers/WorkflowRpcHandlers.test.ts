...
const noopReadModel = {
  ...
  listPackPriorOutputs: () => Effect.succeed([]),
  listPackFailedAttempts: () => Effect.succeed({ rows: [], totalMatched: 0 }),
  ...
  return handler
    ...
    : Effect.fail(
        new WorkflowRpcError({
          message: `${method} handler is not registered`,
        }),
      );
};
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* fileSystem.makeDirectory(path.join(workspaceRoot, ".t3/boards"), {
        recursive: true,
      });
        ...
        workspaceFileSystem: {
          ...
          deleteFile: (input: Parameters<WorkspaceFileSystem["Service"]["deleteFile"]>[0]) =>
            Effect.gen(function* () {
              ...
              yield* fileSystem
                .remove(path.join(input.cwd, input.relativePath), {
                  force: true,
                })
                .pipe(Effect.orDie);
            }),
...
it.effect("workflowRpcHandlers rejects deleteBoard whose derived path is not a board file", () =>
  Effect.gen(function* () {
    ...
    const result = yield* Effect.exit(
      invokeWorkflowHandler(handlers, WORKFLOW_WS_METHODS.deleteBoard, {
        boardId,
      }),
    );
          ...
          failNextVersionRecord
            ? Effect.sync(() => {
                ...
                Effect.andThen(
                  Effect.fail(
                    new WorkflowEventStoreError({
                      message: "version record unavailable",
                    }),
                  ),
                ),
              )
    ...
    const deps = makeProposeDeps({
      ...
      gen: () =>
        Effect.succeed({
          ...
          rationale: "renamed the work lane",
        }),
    });
  ...
  () =>
    Effect.gen(function* () {
      ...
      const deps = makeProposeDeps({
        ...
        gen: () =>
          Effect.succeed({
            proposedDefinition: { not: "a workflow def" },
            rationale: "broken",
          }),
      });
...
it.effect("listBoardProposals handler returns proposals from readModel", () =>
  Effect.gen(function* () {
    ...
    const result = yield* listBoardProposals(deps, {
      boardId: proposalBoardId,
    });
...
it.effect("getBoardProposal handler fails with WorkflowRpcError when not found", () =>
  Effect.gen(function* () {
    ...
    const exit = yield* getBoardProposal(deps, {
        ...
        Effect.gen(function* () {
          const definition = yield* decodeWorkflowDefinitionJson(fileContents).pipe(
            Effect.mapError(
              (cause) =>
                new WorkflowRpcError({
                  message: "round-trip decode failed",
                  cause,
                }),
            ),
          );
...
versionRoundTripLayer("resolveBoardProposal (self-improve E6)", (it) => {
  ...
  it.effect("approve re-runs PRESERVATION and rejects a now-invalid proposal (no save)", () =>
    Effect.gen(function* () {
      ...
      const h = makeResolveHarness({
        versionStore,
        proposedDefinition: proposedDropsLane,
      });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const h = makeResolveHarness({
          versionStore,
          proposedDefinition: proposedDeadEnd,
        });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const result = yield* revertBoardProposal(h.deps, {
          proposalId: "prop-e7-1",
        });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const h = makeRevertHarness({
          versionStore,
          currentFileMatchesProposed: false,
        });
      ...
      pipeline: [
        {
          ...
          retry: {
            maxAttempts: 3,
            escalate: { instance: "esc", model: "opus" },
          },
  ...
  () =>
    Effect.gen(function* () {
      ...
      const deps = makeDraftDeps({
        ...
        gen: () =>
          Effect.succeed({
            proposedDefinition: draftGeneratedDef,
  ...
  () =>
    Effect.gen(function* () {
      ...
      const deps = makeDraftDeps({
        ...
        gen: () =>
          Effect.succeed({
            proposedDefinition: forbiddenDef,
            rationale: "with a merge",
          }),
      });
      ...
      const deps = makeDraftDeps({
        ...
        lint: () =>
          Effect.succeed([
            {
              code: "missing_lane_ref",
              message: "transition to a missing lane",
            } as LintError,
          ]),
      });
  ...
  () =>
    Effect.gen(function* () {
      ...
      const deps = makeDraftDeps({
        ...
        gen: () =>
          Effect.succeed({
            proposedDefinition: badEntryDef,
            rationale: "oops",
          }),
      });
  ...
  () =>
    Effect.gen(function* () {
      ...
      const deps = makeDraftDeps({
        ...
        gen: () =>
          Effect.succeed({
            proposedDefinition: hugeDef,
            rationale: "too many lanes",
          }),
      });
  ...
  () =>
    Effect.gen(function* () {
      ...
      const deps = makeDraftDeps({
        ...
        gen: () =>
          Effect.succeed({
            proposedDefinition: strandingDef,
            rationale: "no way out",
          }),
      });
...
it.effect("generateWorkflowDraft → ok:false when a single lane is too large; no persist", () =>
  Effect.gen(function* () {
    ...
    const deps = makeDraftDeps({
      ...
      gen: () =>
        Effect.succeed({
          proposedDefinition: hugeLaneDef,
          rationale: "huge lane",
        }),
    });
  ...
  () =>
    Effect.gen(function* () {
      ...
      const deps = makeDraftDeps({
        ...
        gen: () =>
          Effect.succeed({
            proposedDefinition: oversizedDef,
            rationale: "too big",
          }),
      });
  ...
  () =>
    Effect.gen(function* () {
      ...
      const deps = makeDraftDeps({
        ...
        gen: () =>
          Effect.succeed({
            proposedDefinition: manyStepsDef,
            rationale: "many steps",
          }),
      });
...
it.effect("gates mutating RPCs behind readiness while reads bypass the gate", () =>
  Effect.gen(function* () {
    ...
    const boards = yield* handlers[WORKFLOW_WS_METHODS.listBoards]({
      projectId,
    });
      ...
      reconcileChunk: (bid, _lanes, deltas) =>
        Effect.sync(() => {
          capturedReconcileCalls.push({
            boardId: String(bid),
            deltas: deltas as ReadonlyArray<{
              _tag: string;
